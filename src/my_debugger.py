    ...
    def get_debug_event(self):
        ...
        if kernel32.WaitForDebugEvent(byref(debug_event),100):
            ...
            if debug_event.dwDebugEventCode == EXCEPTION_DEBUG_EVENT:
                ...
                print "Exception_address: %x" % self.exception_address
                ...
                if self.exception == EXCEPTION_ACCESS_VIOLATION:
                    ...
                    print "skip single step"
             
            if debug_event.dwDebugEventCode == CREATE_PROCESS_DEBUG_EVENT:
                print "Debug Process Created"
                h = CDLL('hidepydbg.dll')
                h.HideDebug(self.process_information)
                #self.bp_set_hw(0x0047673C, 1, HW_ACCESS)
                
            if debug_event.dwDebugEventCode == CREATE_THREAD_DEBUG_EVENT:
                print "New Thread Created. Start: %x" % debug_event.u.CreateThread.lpStartAddress
                #context = self.get_thread_context(thread_id=debug_event.dwThreadId)
                #print "EIP: %x" % context.Eip
                #self.bp_set_hw(0x0047673C, 1, HW_ACCESS)
