...
long __stdcall hd_FindEx(HANDLE hProcess, LPVOID MemoryStart, DWORD MemorySize, LPVOID SearchPattern, DWORD PatternSize, LPBYTE WildCard)
{
	/*
...
// from TitanEngine SDK - Property of ReversingLabs www.reversinglabs.com
long __stdcall hd_Find(LPVOID MemoryStart, DWORD MemorySize, LPVOID SearchPattern, DWORD PatternSize, LPBYTE WildCard)
{
	/*
	...
	*/
	...
	if(hProc != NULL)
	{
		...
		if(OS == Windows7 && hd_IsOSWorkstationEdition())
			odsAddr = (void*)GetProcAddress(GetModuleHandle("kernelbase.dll"), "OutputDebugStringA");
		else
			odsAddr = (void*)GetProcAddress(GetModuleHandle("kernel32.dll"), "OutputDebugStringA");
